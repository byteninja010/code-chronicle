...
- **Webview**: A VS Code webview panel that renders the interactive graph visualization
- **User**: A developer using the CodeChronicle extension
- **AI_Service**: AWS AI services (e.g., AWS Bedrock) used for semantic analysis and natural language processing
...
1. WHEN a User requests risk analysis for a Node, THE Extension SHALL send the file contents to AI_Service
2. WHEN sending to AI_Service, THE Extension SHALL include file contents, Structural_Metrics, and dependency context
3. WHEN AI_Service returns analysis, THE Extension SHALL extract a Risk_Factor score (low, medium, high)
4. WHEN AI_Service returns analysis, THE Extension SHALL extract a human-readable explanation of the risk
5. WHEN AI_Service fails, THE Extension SHALL fall back to Structural_Metrics only and notify the User
...
1. WHEN a User activates the graph view, THE Extension SHALL open a Webview panel
2. WHEN the Webview renders, THE Extension SHALL display the Code_Graph with Nodes and Edges
3. WHEN rendering Nodes, THE Extension SHALL scale node size based on Structural_Metrics
4. WHEN rendering Nodes, THE Extension SHALL color nodes based on Risk_Factor (green for low, yellow for medium, red for high)
...
1. WHEN a User clicks on a Node, THE Extension SHALL request a summary from AI_Service
2. WHEN requesting a summary, THE Extension SHALL provide file contents and Code_Graph context to AI_Service
3. WHEN AI_Service returns a summary, THE Extension SHALL display it in the Webview side panel
4. WHEN a summary is generated, THE Extension SHALL cache it to avoid redundant API calls
5. WHEN AI_Service fails, THE Extension SHALL display Structural_Metrics and raw file metadata instead
...
1. WHEN a User selects a Node and activates Blast_Radius mode, THE Extension SHALL compute all transitive dependents
2. WHEN Blast_Radius is computed, THE Extension SHALL highlight all dependent Nodes in the Webview
3. WHEN Blast_Radius is displayed, THE Extension SHALL request an impact analysis from AI_Service
4. WHEN AI_Service returns impact analysis, THE Extension SHALL display potential breaking changes in the side panel
...
1. WHEN a User submits a natural language query, THE Extension SHALL send the query and Code_Graph context to AI_Service
2. WHEN AI_Service processes the query, THE Extension SHALL return file-level answers with explanations
...
5. WHEN AI_Service cannot answer the query, THE Extension SHALL suggest alternative questions or clarifications
...
1. WHEN the Code_Graph is constructed, THE Extension SHALL serialize it to a cache file in the Workspace
2. WHEN AI summaries are generated, THE Extension SHALL cache them with file content hashes
3. WHEN the Extension is reactivated, THE Extension SHALL load the cached Code_Graph if available
4. WHEN a cached file's content hash changes, THE Extension SHALL invalidate cached AI analysis for that file
5. WHEN the User requests a full refresh, THE Extension SHALL clear all caches and rebuild the Code_Graph
...
1. WHEN AI_Service rate limits are exceeded, THE Extension SHALL queue requests and retry after the limit resets
2. WHEN AI_Service calls fail, THE Extension SHALL return cached results if available or display a user-friendly error message
3. WHEN file parsing fails for specific files, THE Extension SHALL log the error and continue processing remaining files
4. WHEN the Workspace contains more than 10,000 files, THE Extension SHALL warn the User and offer to limit analysis scope
5. WHEN network connectivity is lost, THE Extension SHALL continue providing deterministic features (graph, metrics) without AI
...
4. THE Extension SHALL display a status bar item showing the current analysis state (scanning, ready, error)
5. THE Extension SHALL respect VS Code theme settings when rendering the Webview
...
2. THE Extension SHALL provide a configuration option to specify AWS credentials for AI_Service
3. THE Extension SHALL provide a configuration option to enable or disable AI features
4. THE Extension SHALL provide a configuration option to set the maximum number of files to analyze
